...
export const translations = {
  en: {
    ...
    trackPreProcessing: "Track pre-processing",
    showAdvancedOptions: "Show advanced options",
    resampleSpacing: "Resample spacing (m)",
    resampleSpacingTip: "Places track points at even intervals. Lower values follow tight corners more closely and usually increase measured distance.",
    smoothWindow: "Elevation smooth window (m)",
    smoothWindowTip: "Averages elevation over this distance to remove GPS spikes. Higher values give a smoother profile and lower D+/D−.",
    elevationDeadband: "Elevation deadband (m)",
    elevationDeadbandTip: "Elevation must change by at least this much before it counts as ascent or descent. Reduces inflated D+/D− from barometer noise.",
    ...
    timeMethodActivityBlendDesc:
      "Times each leg point-to-point along the pre-processed track for more precision on rolling terrain, rather than using the table's segment distance and D+/D− totals. Applies the SAC blend to every short step and sums the result. Downhill-dominant steps use the downhill factor.",
    ...
    segmentTimeFormulaActivityBlendCode:
      "per step along the track:\nt_flat = distance_m / flat_speed\nif climbing:  t_vert = ascent_m / ascent_speed\nif descending: t_vert = descent_m / descent_speed\ntime = max(t_flat, t_vert)\n     + 0.5 * min(t_flat, t_vert)\nif descent >= ascent:\n  time *= downhill_factor\nleg time = sum(step times)",
  ...
  fr: {
    ...
    trackPreProcessing: "Pré-traitement de la piste",
    showAdvancedOptions: "Afficher les options avancées",
    resampleSpacing: "Espacement de rééchantillonnage (m)",
    resampleSpacingTip: "Place des points à intervalles réguliers. Une valeur plus basse suit mieux les virages serrés et augmente en général la distance mesurée.",
    smoothWindow: "Fenêtre de lissage altitude (m)",
    smoothWindowTip: "Moyenne l'altitude sur cette distance pour supprimer les pics GPS. Une valeur plus haute lisse le profil et réduit le D+/D−.",
    elevationDeadband: "Seuil altitude (m)",
    elevationDeadbandTip: "L'altitude doit varier d'au moins ce seuil pour compter en montée ou descente. Réduit le D+/D− gonflé par le bruit barométrique.",
    resampling: "Rééchant.",
    smoothing: "Lissage",
    deadband: "Seuil",
    ...
    timeMethodActivityBlendDesc:
      "Chronomètre chaque tronçon point par point le long de la trace pré-traitée pour plus de précision sur terrain vallonné, plutôt que d'utiliser la distance et le D+/D− totaux du tableau. Applique le mélange CAS à chaque petit pas et additionne le résultat. Les pas en descente dominante utilisent le facteur de descente.",
    segmentTimeFormulaAdditiveCode:
    ...
    segmentTimeFormulaActivityBlendCode:
      "par pas le long de la trace :\nt_plat = distance_m / vitesse_plate\nsi montée :  t_vert = montée_m / vitesse_montée\nsi descente : t_vert = descente_m / vitesse_descente\ntemps = max(t_plat, t_vert)\n     + 0,5 * min(t_plat, t_vert)\nsi descente >= montée :\n  temps *= facteur_descente\ntemps tronçon = somme des pas",
  ...
  es: {
    ...
    trackPreProcessing: "Preprocesado de pista",
    showAdvancedOptions: "Mostrar opciones avanzadas",
    resampleSpacing: "Espaciado de remuestreo (m)",
    resampleSpacingTip: "Coloca puntos a intervalos regulares. Valores más bajos siguen mejor las curvas cerradas y suelen aumentar la distancia medida.",
    smoothWindow: "Ventana de suavizado de elevación (m)",
    smoothWindowTip: "Promedia la elevación en esta distancia para eliminar picos GPS. Valores más altos suavizan el perfil y reducen el D+/D−.",
    elevationDeadband: "Banda muerta de elevación (m)",
    elevationDeadbandTip: "La elevación debe cambiar al menos esta cantidad para contar como ascenso o descenso. Reduce el D+/D− inflado por ruido del barómetro.",
    ...
    timeMethodActivityBlendDesc:
      "Cronometra cada tramo punto a punto a lo largo del track preprocesado para más precisión en terreno ondulado, en lugar de usar la distancia y el D+/D− totales de la tabla. Aplica la mezcla SAC a cada paso corto y suma el resultado. Los pasos con bajada dominante usan el factor de descenso.",
    segmentTimeFormulaAdditiveCode:
      "t_llano = distancia_m / vel_llana\nt_subida = ascenso_m / vel_subida\nt_bajada = descenso_m / vel_bajada\ntiempo = t_llano + t_subida + t_bajada",
    segmentTimeFormulaActivityBlendCode:
      "por paso a lo largo del track:\nt_llano = distancia_m / vel_llana\nsi subida:  t_vert = ascenso_m / vel_subida\nsi bajada:  t_vert = descenso_m / vel_bajada\ntiempo = max(t_llano, t_vert)\n     + 0,5 * min(t_llano, t_vert)\nsi descenso >= ascenso:\n  tiempo *= factor_bajada\ntiempo tramo = suma de pasos",
  ...
  ca: {
    ...
    trackPreProcessing: "Preprocessament de la pista",
    showAdvancedOptions: "Mostrar opcions avançades",
    resampleSpacing: "Espaiat de remostreig (m)",
    resampleSpacingTip: "Col·loca punts a intervals regulars. Valors més baixos segueixen millor les corbes tancades i solen augmentar la distància mesurada.",
    smoothWindow: "Finestra de suavitzat d'elevació (m)",
    smoothWindowTip: "Mitjana l'elevació en aquesta distància per eliminar pics GPS. Valors més alts suavizen el perfil i redueixen el D+/D−.",
    elevationDeadband: "Banda morta d'elevació (m)",
    elevationDeadbandTip: "L'elevació ha de canviar almenys aquesta quantitat per comptar com a pujada o baixada. Redueix el D+/D− inflat pel soroll del baròmetre.",
    ...
    timeMethodActivityBlendDesc:
      "Cronometra cada tram punt a punt al llarg del track pre-processat per a més precisió en terreny ondulat, en lloc d'usar la distància i el D+/D− totals de la taula. Aplica la barreja SAC a cada pas curt i suma el resultat. Els passos amb baixada dominant usen el factor de baixada.",
    segmentTimeFormulaAdditiveCode:
    ...
    segmentTimeFormulaActivityBlendCode:
      "per pas al llarg del track:\nt_pla = distancia_m / velocitat_plana\nsi pujada:  t_vert = pujada_m / velocitat_pujada\nsi baixada: t_vert = baixada_m / velocitat_baixada\ntemps = max(t_pla, t_vert)\n     + 0,5 * min(t_pla, t_vert)\nsi baixada >= pujada:\n  temps *= factor_baixada\ntemps tram = suma de passos",
